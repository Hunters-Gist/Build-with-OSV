...
export default function Pipeline() {
  ...
  return (
    <div className="app-shell h-full relative overflow-hidden">
      ...
      <div className="relative z-10 app-container max-w-[1600px]">
        <div className="app-header md:flex-row md:justify-between md:items-end gap-4">
          <div>
            <h1 className="app-title tracking-tight text-osv-white">Pipeline</h1>
            <p className="font-mono text-osv-muted text-fluid-sm mt-2 tracking-wide">TOTAL VALUE: $45,000</p>
          </div>
          <button className="inline-flex items-center justify-center bg-osv-accent text-osv-bg font-medium h-11 px-6 rounded-lg hover:brightness-110 hover:shadow-[0_0_20px_rgba(245,158,11,0.4)] transition-all duration-200 active:scale-[0.98] outline-none focus-visible:ring-2 focus-visible:ring-osv-accent focus-visible:ring-offset-2 focus-visible:ring-offset-osv-bg shrink-0">
            + Manual Lead
          </button>
        </div>

        <div className="flex overflow-x-auto gap-4 md:gap-6 pb-6 h-[calc(100vh-180px)] sm:h-[calc(100vh-210px)] min-h-[460px] md:min-h-[500px] snap-x">
          {STAGES.map(stage => (
            <div key={stage} className="flex-1 min-w-[280px] sm:min-w-[320px] max-w-[400px] bg-osv-panel/40 backdrop-blur-md border border-white/5 rounded-2xl flex flex-col snap-start overflow-hidden">
                  ...
                  return (
                    <div key={lead.id} className={`p-5 rounded-xl border bg-osv-alt/80 cursor-pointer transition-all duration-300 hover:-translate-y-1 hover:shadow-lg ${isStale ? 'border-osv-red/50 hover:border-osv-red hover:shadow-[0_0_15px_rgba(239,68,68,0.15)]' : 'border-white/10 hover:border-osv-accent/40 hover:shadow-[0_0_15px_rgba(245,158,11,0.1)]'}`}>
                      <div className="flex justify-between items-start mb-3 gap-2 min-w-0">
                        <h3 className="min-w-0 text-base font-medium text-osv-white leading-tight wrap-break-word">{lead.client_name || lead.title}</h3>
                      ...
                      <div className="flex justify-between text-sm text-osv-muted mb-4 font-sans gap-3 min-w-0">
                        <span className="min-w-0 truncate">{lead.suburb}</span>
                        <span className="text-osv-white/90 whitespace-nowrap">{lead.budget_range || lead.value}</span>
                      </div>
